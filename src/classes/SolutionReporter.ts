...
export default class SolutionReporter implements iSolutionReporter {
	...
	constructor(compatibilityFinder: iCompatibilityFinder) {
		...
		this.solutionsFromPieceGroups = Array.from(this.compatibilityFinder.solutionsByPieceGroup.values()).map(
			pieceGroupSolutions => pieceGroupSolutions[0]
		);
	}
	...
	private getAllSolutions(compatibilityFinder: iCompatibilityFinder): iSolution[] {
		...
		compatibilityFinder.solutionsByPieceGroup.forEach((solutions: iSolution[]) => solutionsAll.push(...solutions));
		return solutionsAll;
	}
